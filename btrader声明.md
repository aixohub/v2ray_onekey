...
### 指标使用逻辑
1.&ensp;指标始终在策略的\_\_init\_\_期间实例化  
2.&ensp;指标的值（或者指标派生出来的值）在next方法中被使用  

有一个重要的公理要考虑：  
在\_\_init\_\_期间声明的任何指标（或其派生值）将在调用next之前进行预先计算。  
我们来探讨一下操作模式的差异：  

#### \_\_init\_\_ vs next
* 在\_\_init\_\_期间涉及线对象的任何操作都会生成另一个线对象
...
#### 为什是\_\_init\_\_而不是next的原因
逻辑简化易用是关键。可以在\_\_init\_\_期间进行相关的声明和计算，以便在next方法中将实际操作逻辑降至最低。  
还有额外的好处：速度（因为一开始就预先计算好了）。  
一个在\_\_init\_\_期间生成购买信号的完整示例：
```python
class MyStrategy(bt.Strategy):
...
显而易见，在\_\_init\_\_期间使用“声明式”方法可以将next方法中（实际策略触发的地方）的cpu使用降至最低。
...
```
>注意
>在\_\_init\_\_版本中，使用bt.Max分配Line对象给self.lines.dummyline。

bt.Max返回一个line对象，该对象会针对传递到指标的每个柱线自动进行迭代。  
如果使用max代替，则分配将是毫无意义的，因为指标得到一个固定值，而不是一个线对象。
在next方法中，将直接使用浮点值完成工作，并且可以使用内置标准的max函数。  
...
#### 手动/自动最短时间
如果可能，backtrader将对其进行计算，但可能需要手动操作。
这是简单移动平均线的潜在实现：
```python
...
* 另一个移动平均线...这已经有一个周期
&emsp;&emsp;如果这是20，而我们的样本移动平均线也为20，那么我们的最小周期为40条
&emsp;&emsp;实际上，内部计算结果为39...因为第一个移动平均线一旦产生一个柱线，便会计入下一个移动平均线，这会产生重叠的柱线，因此需要39。
* 其他带有期间的指标/目标
...
在类Unix环境下，时间完全取决于C编译器。 在Windows下，甚至安装了特定的Microsoft编译器（在本例中为Python 2.7）可能会失败。 但是，一个知名站点收集了Windows的最新软件包。 如果需要，请访问它：
